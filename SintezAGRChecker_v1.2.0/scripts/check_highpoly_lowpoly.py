...
from .package_helper import ensure_import_pillow
...
class HighpolyChecks():
    ...
    def _check_duplicates(self):
        ...
        for fbx in self._hp_fbx_files:
            ...
            for obj in fbx.meshes:
                ...
                count_doubles, loose_verts, loose_eedges, loose_faces = CheckUtils.get_count_doubles(obj, is_ucx)
                if count_doubles > 0:
                    if "ground" in obj.name.lower():
                        check1.add_error(f"Полигоны с opacity рекомендуется дублировать на расстояние более 0,002 м. При необходимости, провести проверку на дубликаты вручную, т. к. автоматическая проверка находит все дубликаты, в т. ч. по полигонам с opacity")
                    check1.add_error(f"{obj.name}: {count_doubles} шт. дубликатов вершин")
                if loose_verts > 0:
                    check1.add_error(f"{obj.name}: {loose_verts} шт. изолированных вершин")
                if loose_eedges > 0:
                    check1.add_error(f"{obj.name}: {loose_eedges} шт. изолированных ребер")
                if loose_faces > 0:
                    check1.add_error(f"{obj.name}: {loose_faces} шт. изолированных граней")
        return checks
    ...
    def _check_naming_masks(self):
        ...
        for fbx in self._hp_fbx_files:
            ...
            elif self._oks_count == 1 and "Ground" not in fbx.name:
                if adress not in fbx.name:
                    check1.add_error(f"{fbx.name} : адрес ОКС не соответствует адресу благоустройства")
                adress = fbx.name.replace("SM_", "").replace("_Light", "")
                address_with_num = adress
    ...
    def _check_fbx_objects(self):
        checks = []
        # check6 = Check("Проверка файлов. Количество fbx", f"Проверка файлов", True, "таблица 2, п. 1, п/п 2", "", ["2.1.2"])
        # checks.append(check6)
        # if len(self._hp_fbx_files) == 0:
        #     check6.add_error(f"Не найдены fbx-файлы. Проверьте структуру архива и убедитесь отсутствии вложенных папок")
    ...
    def _check_json_properties(self):
        ...
        for fbx in self._hp_fbx_files:
            ...
            if len(coordinates) != 2:
                check2.add_error(f"В поле coordinates должно быть 2 значения (x, y). Проверьте синтаксис. Для десятичного разделителя используйте точку.")
            ...
            with open(os.path.join(root_dir, r"fno_codes_utf8.csv"), encoding='utf-8-sig') as csvfile:
                ...
                if code_len not in [3, 6, 9]:
                    check2.add_error(f"fno_code {fno_code}: код не соответствует шаблонам - XXX, XXX XXX, XXX XXX XXX")
                    return checks
            ...
            for obj in fbx.meshes:
                ...
                if "glass" in obj.name.lower():
                    ...
                    try:
                        ...
                        if len(fbx.json_data["features"][0]["Glasses"]) > 1:
                            wrong_glass_structure = True
                    except Exception as e:
                        check2.add_error(f"{obj.name}: Не удалось получить параметры материалов остекления (возможна ошибка синтаксиса или структура файла не соответствует требованиям)")
                    if json_glasses:
                        for mat in obj.data.materials:
                            if mat.name in json_glasses.keys():
                                try:
                                    ...
                                    _ = float(json_glass['metallicity'].replace(',', '.'))
                                    _ = float(json_glass['roughness'].replace(',', '.'))
                                    _ = float(json_glass['refraction'].replace(',', '.'))
                                    _ = float(json_glass['transparency'].replace(',', '.'))
                                except Exception as e:
                                    # logger.add(f"Не удалось наначить параметры материала остекления ({e})")
                                    check2.add_error(f"{mat.name}: Не удалось получить параметры материала остекления ({e})")
                            else:
                                check2.add_error(f"{mat.name}: В файле geojson не найдено описание материала")

        if wrong_glass_structure:
            check2.add_error('Нарушена структура описания остекления. Структура описанная в требованиях подразумевает несколько открывающих скобок, которые должны закрываться в конце всего списка')
            check2.add_error('    "Glasses": [{    "glass1": {...}, "glass2": {...}    }]')
    ...
    def _check_png(self):
        checks = []
        if ensure_import_pillow():
            from PIL import Image

            for fbx in self._hp_fbx_files:
                tex_set = fbx.udim_set
                if not tex_set:
                    continue
                ...
                checks.append(check3)
                check4 = Check("Проверка png. RGB", fbx.name, True, "", "файлов", ["4.1.2"])
                checks.append(check4)
                check5 = Check("Проверка png. Одноцветные незаглушки", fbx.name, True, "", "файлов", ["4.1.3"])
                checks.append(check5)

                alpha_erm_normal, alpha_plugs, one_color_plugs, rgb_mode_errors = [], [], [], []

                for image_name in tex_set.diffuse_set + tex_set.erm_set + tex_set.normal_set:
                    if image_name == None:
                        continue
                    ...
                    if res == 256:
                        if CheckUtils.image_has_alpha_pil(img_pil):
                            alpha_plugs.append(image_name)
                        if not CheckUtils.is_one_color_image_pil(img_pil, imPath):
                            one_color_plugs.append(image_name)
                    else:
                        colors = img_pil.getcolors(2)
                        if colors and len(colors) == 1:
                            check5.add_error(f"{image_name}")
                        if "Diffuse" in image_name:
                            if "RGB" not in img_pil.mode:
                                rgb_mode_errors.append(image_name)
                        else:
                            if CheckUtils.image_has_alpha_pil(img_pil):
                                alpha_erm_normal.append(image_name)
                            if img_pil.mode != "RGB":
                                rgb_mode_errors.append(image_name)
                
                if alpha_erm_normal:
                    check1.add_error("Текстуры с альфа-каналом:")
                    [check1.add_error(im_name) for im_name in alpha_erm_normal]
                if alpha_plugs:
                    check2.add_error("Текстуры с альфа-каналом:")
                    [check2.add_error(im_name) for im_name in alpha_plugs]
                if one_color_plugs:
                    check3.add_error("Многоцветные заглушки:")
                    [check3.add_error(im_name) for im_name in one_color_plugs]
                if rgb_mode_errors:
                    check4.add_error("Изображения с неверным режимом:")
                    [check4.add_error(im_name) for im_name in rgb_mode_errors]
                ...
                #             if CheckUtils.image_has_alpha_pil(imPath, im):
...
class LowpolyChecks():
    ...
    def _check_naming_masks(self): 
        ...
        for fbx in self._lp_fbx_files:
            ...
            adress = fbx.name
            if adress[:4].isdigit() and adress[4] == "_":
                adress = "_".join(adress.split("_")[1:])
            if adress[-2:].isdigit() and adress[-3] == "_":
                adress = "_".join(adress.split("_")[:-1])
            if "ground" in adress.lower():
                adress = adress.replace("_Ground", "")
                ...
                for mat in obj.data.materials:
                    ...
                    for i, tex_symbol in [(0, "d"), (1, "m"), (2, "r"), (4, "o"), (5, "n")]:
                        ...
                        if node.bl_static_type == 'TEX_IMAGE':
                            ...
                            if node.outputs[1].is_linked or node.image.depth == 32:
    ...
    def _check_duplicates(self):
        ...
        for fbx in self._lp_fbx_files:
            ...
            for obj in fbx.meshes:
                ...
                count_doubles, loose_verts, loose_eedges, loose_faces = CheckUtils.get_count_doubles(obj)
                if count_doubles > 0:
                    check1.add_error(f"{obj.name}: {count_doubles} шт. дубликатов вершин")
                if loose_verts > 0:
                    check1.add_error(f"{obj.name}: {loose_verts} шт. изолированных вершин")
    ...
    def _check_texel_dencity(self):
        ...
        bpy.context.scene.agr_scene_properties.lp_td_errors.clear()
    ...
    def _check_fbx_objects(self):
        ...
        if len(self._lp_fbx_files) == 0:
            check2.add_error(f"Не найдены fbx-файлы. Проверьте структуру архива и убедитесь отсутствии вложенных папок")
...
class CheckUtils():
    ...
    def check_files(check_lp, root_path):
        ...
        for root, dirs, files in os.walk(root_path):
            ...
            for file in files:
                if ".blend" in file:
                    continue
                if os_utils.CHECKLIST_DATA_FOLDER_NAME in root or os_utils.EXPORT_FOLDER_NAME in root:
                    continue
    ...
    @staticmethod
    def get_count_doubles(obj, is_ucx=False):
        # bpy.ops.object.mode_set(mode='OBJECT')
        obj.hide_set(False)
        bpy.context.view_layer.objects.active = obj
        bpy.ops.object.mode_set(mode='EDIT')
        ...
        count_doubles = len(select["targetmap"])

        loose_verts, loose_edges, loose_faces = 0, 0, 0
        if not is_ucx:
            loose_verts = CheckUtils.get_loose_parts_count(bm, "VERT")
            loose_edges = CheckUtils.get_loose_parts_count(bm, "EDGE")
            # loose_faces = CheckUtils.get_loose_parts_count(bm, "FACE")

        return (count_doubles, loose_verts, loose_edges, loose_faces)
    ...
    def check_texel(fbx, obj, check1, check2, check3, highpoly=True, resolution_lowpoly=0):
        if highpoly:
            if all([v == 256 for v in fbx.udim_set.resolutions_by_number.values()]):
                return
            calculated_obj_td_area, td_errors_less, td_errors_greater, udim_errors, udim_used_errors, td_less_indices, td_greater_indices, uv_data = CheckUtils._calculate_td(obj, fbx.udim_set.resolutions_by_number, True)
        else:
            calculated_obj_td_area, td_errors_less, td_errors_greater, udim_errors, udim_used_errors, td_less_indices, td_greater_indices, uv_data = CheckUtils._calculate_td(obj, {1001:resolution_lowpoly}, False)

        if td_less_indices:
            obj["agr_td_less"] = td_less_indices
        if td_greater_indices:
            obj["agr_td_greater"] = td_greater_indices
        
        if not highpoly:
            item = bpy.context.scene.agr_scene_properties.lp_td_errors.add()
            item.obj_name = obj.name
            # item["uv_state"] = uv_data
            if td_less_indices:
                item["agr_td_less"] = td_less_indices
            if td_greater_indices:
                item["agr_td_greater"] = td_greater_indices
    ...
    def _calculate_td(obj, udim_resolutions, highpoly):
        ...
        td_less_indices, td_greater_indices = [], []
        uv_data = []
        ...
        for x in range(0, face_count):
            if not highpoly:
                angle = round(math.degrees(obj.data.polygons[x].normal.angle(vector_up)))
            ...
            try:
                for loop in bm.faces[x].loops:
                    loops.append(loop[bm.loops.layers.uv.active].uv)
                    uv_data.append(loop[bm.loops.layers.uv.active].uv.copy())
            except:
                continue
            ...
            if largest_side == None:
                continue
            if highpoly and largest_side <= 256:
                continue
            ...
            if texel_density < texel_min:
                ...
                td_errors_less[udim_num][1].append(texel_density)
                td_errors_less[udim_num][2].append(gm_area)
                ...
                td_errors_greater[udim_num][1].append(texel_density)
                td_errors_greater[udim_num][2].append(gm_area)
        ...
        return calculated_obj_td_list, td_errors_less, td_errors_greater, out_udim_errors, udim_used_errors, td_less_indices, td_greater_indices, uv_data
    ...
    @staticmethod
    def is_one_color_image_pil(im_pil, im_path):
        im_pil = im_pil.convert("RGB")
        colors = im_pil.getcolors()
        if colors:
            return len(colors) == 1
        else:
            bl_im = bpy.data.images.load(im_path, check_existing=True)
            return CheckUtils.is_one_color_image(bl_im)
    ...
    @staticmethod
    def get_loose_parts_count(bm, select_mode):
...
class ModelPreparer():
    ...
    def import_lowpoly_models(self, firstImport=True):
        if self.by_collections:
            ...
            self.lp_fbx_files.clear()
            if firstImport:
                self._clear_blender_file()
            for item in os.listdir(self.root_path):
                if os.path.isdir(os.path.join(self.root_path, item)) and item[:4].isdigit():
                    root = os.path.join(self.root_path, item)
                    for item2 in os.listdir(os.path.join(self.root_path, item)):
                        if os.path.isdir(os.path.join(self.root_path, item, item2)):
                            continue
                        file = item2
            # for root, dirs, files in os.walk(self.root_path):
            #     for file in files:
    ...
    def import_highpoly_models(self, clear=True):
        if self.by_collections:
            ...
            for item in os.listdir(self.root_path):
                if os.path.isdir(os.path.join(self.root_path, item)) and item.startswith("SM_"):
                    root = os.path.join(self.root_path, item)
                    ...
                    for item2 in os.listdir(root):
                        if os.path.isdir(os.path.join(root, item2)):
                            continue
                        file = item2
            # for root, dirs, files in os.walk(self.root_path):
            #     root_fbx_geojson[root] = [[], None]
            ...
            #     for file in files:
                        ...
                        if file.lower().endswith(".fbx"):
                            ...
                            png_count += 1
                    check1 = Check("Проверка файлов. Состав архива.", item, True, "таблица 2, п. 1, п/п 2", "", ["2.1.2"])
                    self.prepare_checks.append(check1)
                    if fbx_count == 0:
                        check1.add_error("Не найдены fbx-файлы. Проверьте структуру архива и убедитесь отсутствии вложенных папок")
                    elif fbx_count > 2:
                        check1.add_error("Найдено 3 или более fbx-файлов")
                    if json_count == 0:
                        check1.add_error("Не найден geojson-файл")
                    elif json_count > 1:
                        check1.add_error("Более одного geojson файла")
                    if png_count < 3 or png_count > 2100:
                        check1.add_error("Не найдены png-файлы")
    ...
    @exception_handler
    def lp_materials_fix(self):
        for fbx in self.lp_fbx_files:
            for obj in fbx.meshes:
                if obj.type != 'MESH':
                    continue
                for mat in obj.data.materials:
