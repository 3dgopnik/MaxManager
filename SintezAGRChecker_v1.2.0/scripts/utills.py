...
from .package_helper import ensure_import_openpyxl
...
def _get_layer_collection(layer_collection, collection_name):
    if layer_collection.name == collection_name:
        return layer_collection
    for child_layer_collection in layer_collection.children:
        found_layer_collection = _get_layer_collection(child_layer_collection, collection_name)
        if found_layer_collection:
            return found_layer_collection
    return None

def get_layer_collection_by_name(col_name):
    master_layer_collection = bpy.context.view_layer.layer_collection
    target_collection_name = col_name
    found_layer_collection = _get_layer_collection(master_layer_collection, target_collection_name)
    return found_layer_collection