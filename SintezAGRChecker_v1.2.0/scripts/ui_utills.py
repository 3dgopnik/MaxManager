    ...
    for cat in categories:
        ...
        if cat.drow_collection:
            ...
            for i, item in enumerate(cat.collection):
                ...
                if item.req_num == "2.6.1" and item.check_state == "failed":
                    # if is_highpoly:

                    td_row = box.row()
                    td_row.label(text="Выбрать полигоны:")
                    less_op = td_row.operator("agr.hp_select_texel_errors", text=f"< {'512' if is_highpoly else '10'}")
                    less_op.error_less = True
                    less_op.is_highpoly = is_highpoly
                    greater_op = td_row.operator("agr.hp_select_texel_errors", text=f"> {'1706' if is_highpoly else '40'}")
                    greater_op.error_less = False
                    greater_op.is_highpoly = is_highpoly
                    # td_row.operator("agr.hp_select_texel_less", text="< 512")
                    # td_row.operator("agr.hp_select_texel_greater", text="> 1706")
                if is_highpoly:
                    if item.req_num == "2.5.2.2.1а":
                ...
                if not item.req_num.startswith("4."):
                ...
                if item.check_state == "failed":
                    if is_highpoly:
                        if item.req_num == "2.5.3.4":
                            req_row.operator("agr.one_color_plugs_fix", text=f"fix")
                        if item.req_num == "2.5.1.6" or item.req_num == "2.5.3.3":
                            req_row.operator("agr.alpha_fix", text=f"fix")
                        if item.req_num == "2.3.13":
                            op = req_row.operator("agr.doubles_and_loose_fix", text=f"fix")
                            op.is_highpoly = True
                    else:
                        if item.req_num == "2.3.7":
                            op = req_row.operator("agr.doubles_and_loose_fix", text=f"fix")
                            op.is_highpoly = False
...
def drow_label_multiline(context, text, parent, width=200):
    # logger.add(context.region.width)
    chars = int(width / 7)   # 7 pix on 1 character
...
def generate_text_editor(context):
    try:
        ...
        if not text_file:
            ...
            default_texts = []
            for text in bpy.data.texts:
                if "Text" in text.name or "Текст" in text.name:
                    default_texts.append(text)
            text_file = default_texts[-1]
            text_file.name = text_name
        # context.space_data.text = bpy.data.texts[text.name]
