...
class CUSTOM_objectCollection_TD_error(PropertyGroup):
...
class AGRCheckerProperties(PropertyGroup):
        ...
        for col in bpy.data.collections:
            ...
            if not layer_col.exclude:
                if col.name[:4].isdigit():
                    layer_col.hide_viewport = not self.show_lp
    ...
    def swith_show_hp(self, context):
        ...
        for col in bpy.data.collections:
            ...
            if not utills.get_layer_collection_by_name(col.name).exclude:
                if col.name.startswith("SM_") and "Light" not in col.name:
                    for obj in col.objects:
                        if "UCX" not in obj.name and "Glass" not in obj.name:
                            obj.hide_set(not self.show_hp)
                    # col.hide_viewport = not self.show_hp
                    # col.hide_select = self.show_hp
        self.show_glass = self.show_hp
        # self.swith_show_glass(context)
        self.show_lights = self.show_hp
        # self.swith_show_lights(context)
    ...
    def swith_show_lights(self, context):
        logger.add(f"self.show_lights={self.show_lights}")
        for col in bpy.data.collections:
            layer_col = utills.get_layer_collection_by_name(col.name)
            if not layer_col.exclude:
                if "light" in col.name.lower():
                    layer_col.hide_viewport = not self.show_lights
    ...
    show_lights: BoolProperty(default=True, update=swith_show_lights)
    ...
    lp_td_errors: CollectionProperty(type=CUSTOM_objectCollection_TD_error)